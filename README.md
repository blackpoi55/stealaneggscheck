...
ตอน deploy จริง (Vercel/อื่น ๆ) ต้องใส่ตัวแปรทุกตัวในหน้า Environment Variables ของโฮสต์เอง
เพราะ `.env.local` ไม่ได้ถูก push ขึ้น git และอย่าลืมเปลี่ยน `NEXT_PUBLIC_SITE_URL` เป็นโดเมนจริง

**ตัวแปรที่ขึ้นต้นด้วย `NEXT_PUBLIC_` ถูกฝังตอน build** ฉะนั้นเพิ่มทีหลังต้อง **redeploy** ถึงจะมีผล

เช็กว่าเซิร์ฟเวอร์มีอะไรครบแล้วบ้างได้ที่ `GET /api/health` — บอกเฉพาะว่า "มี/ไม่มี" ไม่เปิดเผยค่า
...
3. ตั้ง cron ให้ยิงมาที่ endpoint นี้
   ```
   POST https://โดเมนของคุณ/api/push/send
   Header: x-cron-secret: <CRON_SECRET>
   Crontab: 29,59 * * * *
   ```
   แนะนำ [cron-job.org](https://cron-job.org) — ฟรี ตั้ง timezone เป็น Asia/Bangkok ได้
   (Vercel Cron บนแพ็กเกจ Hobby ยิงได้แค่วันละครั้ง ใช้กับงานนี้ไม่ได้)

**ทำไมเป็น `29,59`** เพราะประตูเปิดนาทีที่ :00 กับ :30 ยิงตอนนาทีที่ 29 และ 59 จึงตรงกับ "เหลือ 1 นาที" พอดี
ใช้แค่ 2 ครั้งต่อชั่วโมงแทนที่จะยิงทุกนาที

ถึงอย่างนั้น **ตารางเวลาจริงอยู่ในโค้ด ไม่ได้อยู่ที่ cron** — endpoint จะส่งก็ต่อเมื่อเหลือไม่เกิน 100 วินาที
และตาราง `push_log` กันไม่ให้ส่งซ้ำรอบเดิม ฉะนั้นจะตั้ง cron ให้ยิงทุกนาทีก็ได้ ไม่มีผลเสีย
