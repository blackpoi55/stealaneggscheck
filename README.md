...
- **ตัวจับเวลา Rift** นับถอยหลังว่าอีกกี่นาทีจะเปลี่ยนชุด บอกว่าตอนนี้ชุดไหน ชุดหน้าคืออะไร พร้อมเสียงเตือนล่วงหน้า 1 นาที (ดีฟอลต์ปิด)
...
| `rift_anchor` | จุดอ้างอิงเวลาหมุนของ Rift (1 แถวเท่านั้น) |

### ตัวจับเวลา Rift

ไข่ริฟต์เปลี่ยนชุดทุก 3 ชั่วโมง เว็บคำนวณตารางเองจาก "จุดอ้างอิง" ที่เก็บไว้ในตาราง `rift_anchor`

ตั้งครั้งเดียวที่ `/admin`: เปิดเกมดูว่านับถอยหลังเหลืออีกกี่นาที และตอนนี้เป็นชุดที่เท่าไหร่ แล้วกดบันทึก
จากนั้นเว็บคำนวณต่อเองไปเรื่อย ๆ ไม่ต้องมาตั้งซ้ำ (ถ้าเพี้ยนก็ตั้งใหม่ได้ทุกเมื่อ)
...
**ข้อสมมติ:** ลำดับชุดถือว่าวน 1 → 2 → 3 → 1 ถ้าเกมจริงไม่ใช่แบบนี้ ตั้งค่าใหม่ให้ตรงได้ที่ `/admin`
